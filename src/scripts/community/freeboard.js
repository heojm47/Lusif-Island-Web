...
async function displayPosts(page = 1, sortBy = 'latest') {
    try {
        // 검색 매개변수 추가
        let url = `/api/posts?page=${page}&sortBy=${sortBy}`;
        ...
        if (data.posts.length === 0) {
            ...
            updatePagination(1, 1, sortBy); // 검색 결과가 없을 때 페이지네이션 업데이트
            return;
        }
        ...
        data.posts.forEach(post => {
            ...
            postElement.innerHTML = `
                <div class="post-title">
                    ${icon}<a href="view_post.html?id=${post.id}">${post.title}${commentDisplay}</a>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // 초기 게시글 로드
    displayPosts(1, 'latest');

    // 정렬 변경 이벤트 리스너
    document.addEventListener('sortChange', function(e) {
        displayPosts(1, e.detail.sortBy);
    });
...
async function handleSearch() {
    ...
    try {
        ...
        await displayPosts(1, document.querySelector('input[name="sort"]:checked').value);
    } catch (error) {
...
function resetSearch() {
    ...
    displayPosts(1, document.querySelector('input[name="sort"]:checked').value);
}