document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        // 게시글 데이터 가져오기
        const postResponse = await fetch(`/api/strategy-posts/${postId}`);
        ...
        document.getElementById('post-date').textContent = new Date(responseData.created_at).toLocaleString('ko-KR', {
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
            hour: '2-digit',
        ...
        document.getElementById('edit-button').addEventListener('click', async () => {
            window.location.href = `strategy_edit.html?id=${postId}`;
        });
        ...
        document.getElementById('like-button').addEventListener('click', async function() {
            try {
                const response = await fetch(`/api/strategy-posts/${postId}/like`, {
                    method: 'POST'
                });
        ...
        document.getElementById('post-category').textContent = categoryText;
        document.getElementById('post-category').dataset.category = responseData.category;
